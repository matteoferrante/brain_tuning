...
def organize_results(loader, brain_model):
    ...
    results = defaultdict(lambda: {"y_pred": [], "y_gt": [], "gt_texts": []})
...
def evaluate_and_log(loader, brain_model):
    ...
    for subject_index, subject_data in results.items():
        ...
        gt_texts = subject_data["gt_texts"]
        ...
        # Log a sample of 10 texts and top-5 retrievals for each subject
        sample_indices = np.random.choice(len(y_gt_np), 10, replace=False)
        subject_table = wandb.Table(columns=["Stimulus Text", "Top 5 Retrieved Texts"])

        for i in sample_indices:
            sim_scores = similarity_matrix[i]
            top5_indices = np.argsort(-sim_scores)[:5]
            top5_texts = [gt_texts[idx] for idx in top5_indices]
            stimulus_text = gt_texts[i]
            subject_table.add_data(stimulus_text, top5_texts)
